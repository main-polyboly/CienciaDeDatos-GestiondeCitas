...
public class Main {
    ...
    //main, aqi mostramos los menus de inicio
    public static void main(String[] args) {
        ...
        if (!existeUsername("admin")) {
            ...
            usuarios.add(adm);
            ExcelDB.actualizarCredencial("admin.cred","admin", cred);
        }
    ...
    //aqui autenticamos y verificamos si el usuario que ingresa esta en nuestra base de datos
    static void login() {
        System.out.println("\n─── INICIAR SESION ───");
    ...
    //en caso de que un usuario no este en nuestro sistema, puede registrarse y crear su usuario
    ...
    //  aqui mostramos los menus dependiendo el rol que tenga el usuario
    ...
    // vemos el historial de nuestro paciente (medicos)
    ...
    static void gestionarHistorialMedico(Medico m) {
        System.out.println("\n─── GESTIONAR HISTORIAL DE PACIENTE ───");
        System.out.println("  Buscar por: nombre, cédula, username o código HIS-...");
        System.out.print("  Término de búsqueda: ");
        String termino = sc.nextLine().trim();

        HistorialMedico h = null;

        // Intento exacto por código HIS-
        if (termino.toUpperCase().startsWith("HIS-")) {
            h = buscarHistorialPorCodigo(termino);
        }

        // Si no encontró, buscar en lista
        if (h == null) {
            List<HistorialMedico> res = Buscador.buscarHistoriales(historiales, termino);
            if (res.isEmpty()) {
                System.out.println("  ✗ No se encontró ningún historial con ese término.");
                return;
            }
            if (res.size() == 1) {
                h = res.get(0);
            } else {
                System.out.printf("  Se encontraron %d resultados:%n", res.size());
                for (int i = 0; i < res.size(); i++)
                    System.out.printf("  [%d] %-12s  %-26s  %s%n",
                            i, res.get(i).codigoHistorial,
                            res.get(i).nombrePaciente, res.get(i).cedula);
                System.out.print("  Seleccione índice: ");
                int idx = leerEntero();
                if (idx < 0 || idx >= res.size()) { System.out.println("  Cancelado."); return; }
                h = res.get(idx);
            }
        }

        System.out.println("\n  Paciente : " + h.nombrePaciente);
        System.out.println("  Cédula   : " + h.cedula);
        System.out.println("  Código   : " + h.codigoHistorial);
        System.out.println("  Consultas: " + h.entradas.size());
    ...
    static void filtrarCitasAdmin() {
        System.out.println("  Estado: 1=Confirmada 2=Completada 3=Cancelada 4=No asistió");
        System.out.print("  Seleccione: ");
        int op = leerEntero();
        String[] estados = {"Confirmada","Completada","Cancelada","No asistió"};
    ...
    static void mostrarKPIs() {
        ...
        System.out.printf("  │  No asistió   : %d (%.1f%%)%n", noAsis, tasaNoAs);
